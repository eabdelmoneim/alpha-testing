...
const WalletSelection = (props) => {
    ...
    return (_jsxs(WalletList, { style: {
            minHeight: "100%",
                ...
                return (_jsx("li", { children: wallet.id === "embedded" && modalSize === "compact" ? (_jsx(Suspense, { fallback: _jsx(LoadingScreen, { height: "195px" }), children: _jsx(EmbeddedWalletSelectionUI, { done: () => props.done(wallet), select: () => props.selectWallet(wallet), wallet: wallet, goBack: props.goBack }) })) : (_jsx(WalletEntryButton, { walletId: wallet.id, selectWallet: () => {
                            props.selectWallet(wallet);
                        } })) }, wallet.id));
