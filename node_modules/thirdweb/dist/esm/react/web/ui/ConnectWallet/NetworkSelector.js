...
import { spacing, iconSize, fontSize, radius, media, } from "../design-system/index.js";
...
function NetworkSelectorContentInner(props) {
    ...
    return (_jsxs(Container, { children: [_jsx(Container, { p: "lg", children: props.onBack ? (_jsx(ModalHeader, { title: locale.title, onBack: props.onBack })) : (_jsx(ModalTitle, { children: locale.title })) }), props.showTabs !== false && (_jsxs(_Fragment, { children: [_jsx(Container, { px: "lg", children: _jsxs(Container, { flex: "row", gap: "xxs", children: [_jsx(TabButton, { onClick: () => setSelectedTab("all"), "data-active": selectedTab === "all", children: locale.allNetworks }), _jsx(TabButton, { onClick: () => setSelectedTab("mainnet"), "data-active": selectedTab === "mainnet", children: locale.mainnets }), _jsx(TabButton, { onClick: () => setSelectedTab("testnet"), "data-active": selectedTab === "testnet", children: locale.testnets })] }) }), _jsx(Spacer, { y: "lg" })] })), props.showTabs === false && _jsx(Spacer, { y: "xxs" }), _jsx(Container, { px: "lg", children: _jsxs("div", { style: {
                        ...
                        position: "relative",
                                ...
                                position: "absolute",
                                right: spacing.md,
                            }, children: _jsx(Spinner, { size: "md", color: "accentText" }) }))] }) }), _jsx(Spacer, { y: "lg" }), _jsx(Container, { px: "md", children: _jsx(NetworkTabContent, { allChainIds: allChainsToShow, popularChainIds: popularChainsToShow, recentChainIds: recentChainsToShow, onSwitch: handleSwitch, renderChain: props.networkSelector?.renderChain }) }), onCustomClick && (_jsxs(_Fragment, { children: [_jsx(Line, {}), _jsx(Container, { p: "lg", children: _jsx(Button, { fullWidth: true, variant: "link", onClick: () => {
                                onCustomClick();
                            }, style: {
                                display: "flex",
...
const NetworkTabContent = (props) => {
    const locale = useWalletConnectionCtx().connectLocale.networkSelector.categoryLabel;
    ...
    return (_jsxs(Container, { scrollY: true, animate: "fadein", style: {
        ...
        }, children: [recentChainIds && recentChainIds.length > 0 && (_jsxs("div", { children: [_jsx(SectionLabel, { children: locale.recentlyUsed }), _jsx(Spacer, { y: "sm" }), _jsx(NetworkList, { chains: recentChainIds, onSwitch: props.onSwitch, renderChain: props.renderChain, close: props.close }), _jsx(Spacer, { y: "lg" })] })), popularChainIds && popularChainIds.length > 0 && (_jsxs("div", { children: [_jsx(SectionLabel, { children: locale.popular }), _jsx(Spacer, { y: "sm" }), _jsx(NetworkList, { chains: popularChainIds, onSwitch: props.onSwitch, renderChain: props.renderChain, close: props.close }), _jsx(Spacer, { y: "lg" })] })), ((popularChainIds && popularChainIds.length > 0) ||
                (recentChainIds && recentChainIds.length > 0)) && (_jsxs(_Fragment, { children: [_jsx(SectionLabel, { children: locale.others }), _jsx(Spacer, { y: "sm" })] })), _jsx(NetworkList, { chains: allChainIds, onSwitch: props.onSwitch, renderChain: props.renderChain, close: props.close }), noChainsToShow && (_jsxs(Container, { flex: "column", gap: "md", center: "both", color: "secondaryText", children: [_jsx(Spacer, { y: "xl" }), _jsx(CrossCircledIcon, { width: iconSize.xl, height: iconSize.xl }), _jsx(Text, { children: " No Results " })] }))] }));
};
const NetworkList = /* @__PURE__ */ memo(function NetworkList(props) {
    ...
    return (_jsx(NetworkListUl, { children: chainsToShow.map((chain, i) => {
            ...
            return (_jsx("li", { ref: isLast ? lastItemRef : undefined, children: RenderChain ? (_jsx(RenderChain, { switchChain: () => {
                        handleSwitch(chain);
                    }, chain: chain, switching: switchingChainId === chain.id, switchFailed: errorSwitchingChainId === chain.id, close: props.close })) : (_jsx(ChainButton, { chain: chain, confirming: confirming, onClick: () => handleSwitch(chain), switchingFailed: switchingFailed })) }, chain.id));
        }) }));
});
export const ChainButton = /* @__PURE__ */ memo(function ChainButton(props) {
    ...
    const chainName = apiChainQuery.data ? (_jsxs("span", { children: [apiChainQuery.data.name, " "] })) : (_jsx(Skeleton, { width: "150px", height: "20px" }));
    return (_jsxs(NetworkButton, { "data-active": activeChain?.id === chain.id, onClick: props.onClick, children: [apiChainQuery.data ? (_jsx(ChainIcon, { chain: apiChainQuery.data, size: iconSize.lg, active: activeChain?.id === chain.id, loading: "lazy" })) : (_jsx(Skeleton, { width: iconSize.lg + "px", height: iconSize.lg + "px" })), confirming || switchingFailed ? (_jsxs("div", { style: {
                    display: "flex",
